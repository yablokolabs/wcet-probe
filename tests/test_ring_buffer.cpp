...
void test_push_and_read() {
    ...
    if (rb.size() != 3) {
        std::fprintf(stderr, "  ✗ size\n");
        return;
    }
    if (rb[0] != 10) {
        std::fprintf(stderr, "  ✗ rb[0]\n");
        return;
    }
    if (rb[2] != 30) {
        std::fprintf(stderr, "  ✗ rb[2]\n");
        return;
    }
...
void test_overwrite() {
    ...
    if (rb.size() != 3) {
        std::fprintf(stderr, "  ✗ size after overwrite\n");
        return;
    }
    if (rb[0] != 3) {
        std::fprintf(stderr, "  ✗ oldest should be 3, got %d\n", rb[0]);
        return;
    }
    if (rb[2] != 5) {
        std::fprintf(stderr, "  ✗ newest should be 5, got %d\n", rb[2]);
        return;
    }
...
void test_clear() {
    ...
    rb.clear();
    if (!rb.empty()) {
        std::fprintf(stderr, "  ✗ not empty after clear\n");
        return;
    }
