...
static void sensor_fusion_128() {
    ...
    for (int i = 0; i < 128; ++i) {
        ...
        wx += sensors[i] * w;
        if (i < 64)
            wy += sensors[i] * std::sin(static_cast<double>(i));
        else
            wz += sensors[i] * std::cos(static_cast<double>(i));
    }
    if (sum > 0) {
        result[0] = wx / sum;
        result[1] = wy / sum;
...
int main(int argc, char *argv[]) {
    ...
    std::fprintf(stderr,
                 ...
                 "  Cache flush:  %s\n"
                 "  Trace:        %s\n\n",
                 config.iterations, config.warmup, config.cpu_pin >= 0 ? "yes" : "no",
                 config.flush_cache ? "yes" : "no", config.trace_path ? config.trace_path : "none");
